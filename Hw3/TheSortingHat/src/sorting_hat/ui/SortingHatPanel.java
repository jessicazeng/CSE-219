...
public class SortingHatPanel extends JPanel
{
    ...
    public void renderStats(Graphics g)
    {
                ...
                && data.inProgress() || data.isPaused())
        {
            ...
            String count = "" + data.getBadSpellsCounter();
            int a = TILE_COUNT_X + TILE_COUNT_OFFSET;
            int b = TILE_COUNT_Y + TILE_TEXT_OFFSET;
            g.drawString(count, a, b);
            ...
            if(algorithm.matches("BUBBLE_SORT")){
                algorithm = BUBBLE_SORT;
            }
            if(algorithm.matches("SELECTION_SORT")){
                algorithm = SELECTION_SORT;
            }
            int c = TEMP_TILE_OFFSET_X + TEMP_TILE_X;
            int d = TEMP_TILE_OFFSET_Y + 30;
            g.drawString(algorithm, c, d);
        }        
        ...
        if (game.getGUIDialogs().get(STATS_DIALOG_TYPE).getState().equals(SortingHatTileState.VISIBLE_STATE.toString()))
        {
            ...
            String perfectTime = record.getPerfectWinTime(currentLevel);
            ...
            g.drawString(fastestWinPrompt + perfectTime,                STATS_LEVEL_X, STATS_FASTEST_PERFECT_WIN_Y);
        }
    }
