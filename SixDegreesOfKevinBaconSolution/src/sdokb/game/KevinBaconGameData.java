...
public class KevinBaconGameData
{
    ...
    /**
     * This method processes the guess argument, updating this game accordingly.
     * 
     * @param guess A valid guess for the current game.
     */
    public void guess(IMDBObject guess)
    {
        // ONLY PROCESS THE GUESS IF KEVIN BACON HASN'T ALREADY BEEN FOUND
        if (!isKevinBaconFound())
        {
            String guessId = guess.getId();
            
            // ARE WE EXPECTING AN ACTOR OR FILM?
            if (isWaitingForFilm())
            {
                // IF IT'S THE FIRST NODE, USE THE STARTING ACTOR
                String actor1Id;
                if (lastNode == null)
                {
                    actor1Id = startingActor.getId();
                    guessMap.put(actor1Id, startingActor);
                }
                else
                {
                    actor1Id = lastNode.getActor2Id();
                }
                lastNode = new Connection(actor1Id, guessId);
                gamePath.add(lastNode);
            }
            // IT'S THE SECOND ACTOR
            else
            {
                lastNode.setActor2Id(guessId);
            }
            // RECORD THE GUESS
            guessMap.put(guessId, guess);
