...
public class SortingHatDataModel extends MiniGameDataModel
{
    ...
    public void playWinAnimation()
    {
        ...
        // THIS HAS THE APPROXIMATE PATH  S, WHICH WE'LL SLIGHTLY
        // RANDOMIZE FOR EACH TILE FOLLOWING THE PATH.
        //winPath.add(viewport.getScreenWidth() - 700);
        ...
        winPath.add(viewport.getScreenHeight() - WIN_PATH_COORD);
        winPath.add(viewport.getScreenWidth() - WIN_PATH_COORD);
        winPath.add(viewport.getScreenHeight() - 450);
        winPath.add(WIN_PATH_COORD);
        winPath.add(viewport.getScreenHeight() - 450);
        winPath.add(viewport.getScreenWidth() - WIN_PATH_COORD);
        winPath.add(viewport.getScreenHeight() - WIN_PATH_COORD);
    ...
    public SortTransaction getPreviousSwap(){
        transactionCounter--;
        return properTransactionOrder.get(transactionCounter);
    }
    ...
    public void swapPreviousTiles(int index1, int index2)
    {
        // GET THE TILES
        SortingHatTile tile1 = tilesToSort.get(index1);
        SortingHatTile tile2 = tilesToSort.get(index2);
    ...
    public void endGameAsWin()
    {
        ...
        if(getBadSpellsCounter() == 0){
            ...
            // NEED TO COMPARE TIMES AND ADD SHORTER TIME TO RECORDS
            String time = ((SortingHatMiniGame) miniGame).getPlayerRecord().getPerfectWinTime(currentLevel);
            long timeInMillis = endTime.getTimeInMillis() - startTime.getTimeInMillis();
            long perfectTimeInMillis = ((SortingHatMiniGame) miniGame).getPlayerRecord().getPerfectWinInMillis(currentLevel);
            
            if(time == ""){
                //((SortingHatMiniGame) miniGame).getPlayerRecord().addPerfectTime(currentLevel, gameTimeToText());
                ((SortingHatMiniGame) miniGame).getPlayerRecord().addPerfectWinInMillis(currentLevel, gameTime);
            } else{
                if(timeInMillis < perfectTimeInMillis){
                    //((SortingHatMiniGame) miniGame).getPlayerRecord().addPerfectTime(currentLevel, gameTimeToText());
                    ((SortingHatMiniGame) miniGame).getPlayerRecord().addPerfectWinInMillis(currentLevel, gameTime);
                }
            }
        ...
        try
        {
            ...
            if (inProgress())
            {
                ...
                for(int i=0; i<tilesToSort.size(); i++){
                    ...
                    if(tile.getState().matches(SortingHatTileState.VISIBLE_STATE.toString())){
                        if(tile.containsPoint(getLastMouseX(), getLastMouseY()))
                            tile.setState(SortingHatTileState.MOUSE_OVER_STATE.toString());
                    } else if(tile.getState().matches(SortingHatTileState.MOUSE_OVER_STATE.toString())){
                        if(!tile.containsPoint(getLastMouseX(), getLastMouseY()))
                            tile.setState(SortingHatTileState.VISIBLE_STATE.toString());
                    }
                }
