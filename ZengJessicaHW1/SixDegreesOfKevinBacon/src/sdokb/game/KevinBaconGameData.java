...
public class KevinBaconGameData
{
    ...
    // USED TO KEEP TRACK OF WHETHER THE CURRENT GAME IS A WIN, 
    // LOSS OR PERFECT WIN
                                ...
                                ArrayList<Connection> initShortestPath)
    {
        ...
        // INITIALIZES THE GAME STATUS TO FALSE
        loss = false;
        win = false;
        perfectWin = false;
    }
    ...
    //MUTATOR METHODS
    public void setConnection(IMDBObject actor1, IMDBObject film){
        lastNode = new Connection(actor1.getId(), film.getId());
    }
    
    public void setConnection2(IMDBObject actor1, IMDBObject film, IMDBObject actor2){
        lastNode = new Connection(actor1.getId(), film.getId(), actor2.getId());
    }
    
    public void addToMap(String id, IMDBObject guess){
        guessMap.put(id, guess);
    }
    
    public void addDegree(Connection path){
        if(isWaitingForFilm()){
            int i = getDegrees() - 1;
            gamePath.remove(i);
            gamePath.add(path);
    ...
    public void setWaitingForFilm(boolean value){
        waitingForFilm = value;
    }
    ...
    /**
     * Accessor method for the paths the user has taken in the game.
     * 
     * @return A list of all the connections the user has made.
     */
    public ArrayList<Connection> getDegreeConnections(){
        return gamePath;
    }
