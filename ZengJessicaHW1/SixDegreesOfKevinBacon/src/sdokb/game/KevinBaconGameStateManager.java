 ...
 *
 * @author Richard McKenna & Jessica Zeng
 */
public class KevinBaconGameStateManager
{
    ...
    // KEEPS COUNT OF THE GAMES LOST AND WON
    ...
    public KevinBaconGameStateManager(KevinBaconUI initUI)
    {
        ...
        // INITIALIZES THE GAMES WON AND LOST TO ZERO
        gamesLost = 0;
        gamesWon = 0;
        perfectWins = 0;
    }
    ...
    public void startNewGame()
    {
        ...
        {
            // QUIT THE GAME, WHICH SETS THE END TIME AND INCREMENTS GAMES LOST
            gamesLost++;
    ...
    public void processGuess(IMDBObject guess) throws
            DeadEndException
    {
        // ADDS CURRENT GUESS TO CURRENT GAME MAP
        gameInProgress.addToMap(guess.getId(), guess);
        ...
        if (nonCircularEdges.isEmpty())
        {
            ...
            if(gameInProgress.isWaitingForFilm()){
                if(gameInProgress.getLastConnection() == null){
                    Actor actor1 = gameInProgress.getStartingActor();
                    gameInProgress.setConnection(actor1, guess);
                    gameInProgress.setWaitingForFilm(false);
                } else{
                    Actor actor1 = gameGraphManager.getActor(gameInProgress.getLastConnection().getActor2Id());
                    gameInProgress.setConnection(actor1, guess);
                    gameInProgress.setWaitingForFilm(false);
                }
            }else{
                Actor actor1 = gameGraphManager.getActor(gameInProgress.getLastConnection().getActor1Id());
                Film film = gameGraphManager.getFilm(gameInProgress.getLastConnection().getFilmId());
                gameInProgress.setConnection2(actor1, film, guess);
                gameInProgress.setWaitingForFilm(true);
            }
            gameInProgress.addDegree(gameInProgress.getLastConnection());
        ...
        else{ //change combo box
            if(gameInProgress.isWaitingForFilm()){ // player selected film
                gameInProgress.setWaitingForFilm(false);
                ui.reloadComboBox(nonCircularEdges); //updates combobox
            ...
            else{ // player selected actor
                gameInProgress.setWaitingForFilm(true);
                ui.reloadComboBox(nonCircularEdges); // reload combobox
                ...
                gameInProgress.setConnection2(actor1, film, guess);
                 ...
                 //GAME IS A WIN
                Actor kevinBacon = gameGraphManager.getKevinBacon();
                if((gameInProgress.getLastConnection().getActor2Id()).equals(kevinBacon.getId())){
                    gamesWon++;
                    gameInProgress.endGameAsWin(kevinBacon);
                    
                    // CHECK IF GAME IS PERFECT WIN
            ...
            //adds path; keeps track of degrees
            gameInProgress.addDegree(gameInProgress.getLastConnection());
