...
public class KevinBaconDocumentManager
{
    ...
    private final String GAME_TEXT_LIST_ID = "win_text_list";
    ...
    public void updateGuessesList()
    {
        ...
        try
        {
            // Clear game page then reprint all the connections
            clearGamePage();
            
            Element ol = gameDoc.getElement(GUESSES_LIST_ID);
            
            for(int i=0; i<gameInProgress.getDegrees(); i++){
                String id = gameInProgress.getDegreeConnections().get(i).getActor2Id();
                
                String liText;
                if(id == null){ // Film was selected; need to print film guess
                    liText = START_TAG + HTML.Tag.LI + END_TAG
                            + graph.getActor(gameInProgress.getDegreeConnections().get(i).getActor1Id()) + DASHES + "\""
                            + graph.getFilm(gameInProgress.getDegreeConnections().get(i).getFilmId()) + "\""
                            + START_TAG + SLASH + HTML.Tag.LI + END_TAG;
                    gameDoc.insertBeforeEnd(ol, liText);
                }else{ // Actor selected; print the complete connection
                    liText = START_TAG + HTML.Tag.LI + END_TAG
                            + graph.getActor(gameInProgress.getDegreeConnections().get(i).getActor1Id()) + DASHES + "\""
                            + graph.getFilm(gameInProgress.getDegreeConnections().get(i).getFilmId()) + "\"" + DASHES
                            + graph.getActor(gameInProgress.getDegreeConnections().get(i).getActor2Id())
                            + START_TAG + SLASH + HTML.Tag.LI + END_TAG;
                    gameDoc.insertBeforeEnd(ol, liText);
                }
            }
            
            // Display win message if Kevin Bacon is found
            if(gameInProgress.isKevinBaconFound()){
                Element win = gameDoc.getElement(WIN_DISPLAY_ID);
                String winText = "You Win!";
                gameDoc.insertBeforeEnd(win, winText);
            }
        } 
    ...
    public void addGameResultToStatsPage(KevinBaconGameData completedGame)
    {
        ...
        try
        {
            ...
            statsDoc.setInnerHTML(lossesElement, EMPTY_TEXT + gsm.getGamesLost());
            
            Element winsElement = statsDoc.getElement(WINS_ID);
            statsDoc.setInnerHTML(winsElement, EMPTY_TEXT + gsm.getGamesWon());
            
            Element perfectWinsElement = statsDoc.getElement(PERFECT_WINS_ID);
            statsDoc.setInnerHTML(perfectWinsElement, EMPTY_TEXT + gsm.getPerfectWins());
            
            
            // Print all game results in the current session
            Iterator<Connection> game = gsm.getGameInProgress().gamePathIterator();
            
            String text = "";
            while(game.hasNext()){
                Connection connection = game.next();
                text += "---" + graph.getActor(connection.getActor1Id())
                        + "---" + graph.getFilm(connection.getFilmId());
            }
            
            Element ol = statsDoc.getElement(GAME_RESULTS_LIST_ID);
            if(gsm.getGameInProgress().isLoss()){ // game result if player lost the round
                String liText;
                if(gsm.getGameInProgress().getLastConnection() == null){
                    liText = START_TAG + HTML.Tag.LI + END_TAG
                            + "(0:00:00)"
                            + "/" + gsm.getGameInProgress().getDegrees()
                            + "---" + gsm.getGameInProgress().getStartingActor()
                            + START_TAG + SLASH + HTML.Tag.LI + END_TAG;
                } else{
                    liText = START_TAG + HTML.Tag.LI + END_TAG
                            + "(0:00:00)"
                ...
                statsDoc.insertBeforeEnd(ol, liText);
            } else if(gsm.getGameInProgress().isWin()){ // game result if palyer won the round
                String liText;
                if(gsm.getGameInProgress().isPerfectWin()){ // prints perfect win
                    liText = START_TAG + HTML.Tag.LI + END_TAG + "<font color=\"red\">"
                            + gsm.getGameInProgress().getGameTimeDescription()
                            + "/" + gsm.getGameInProgress().getDegrees()
                            + text
                            + DASHES + "Kevin Bacon"
                            + "</font>" + START_TAG + SLASH + HTML.Tag.LI + END_TAG;
                }else{
                    liText = START_TAG + HTML.Tag.LI + END_TAG + "<font color=\"green\">"
                            + gsm.getGameInProgress().getGameTimeDescription()
                            + "/" + gsm.getGameInProgress().getDegrees()
                            + text
                            + DASHES + "Kevin Bacon"
